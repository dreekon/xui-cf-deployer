...
# xray-cf-lite 最小化部署脚本

`xray_cf_lite.sh` 是一个独立的 Bash 脚本，跳过 3x-ui 面板，直接使用 xray-core 配合 Cloudflare 部署节点。适用于内存极小或不想装面板的服务器。

## 与主脚本的区别

| | xui_cf_deployer.py | xray_cf_lite.sh |
|---|---|---|
| 依赖 | Python 3 + 3x-ui 面板 | Bash + curl + jq |
| 服务管理 | 通过 3x-ui（systemd） | 直接管理 xray（systemd / OpenRC） |
| 写入方式 | 数据库直写 / 面板 API | 直接生成 xray config.json |
| NAT 支持 | 否 | 是（内外端口分离 + 一键换端口） |
| 适用场景 | 有面板的标准 VPS | NAT 小鸡、Alpine 容器、低配机器 |

## 前置条件

- root 权限
- Bash 4+（Alpine 需 `apk add bash`）
- Cloudflare 账号邮箱 + Global API Key
- 脚本会自动安装 `curl`、`jq`、`unzip` 和 `xray-core`

## 运行命令

```bash
curl -fsSL -o xray_cf_lite.sh https://raw.githubusercontent.com/byJoey/xui-cf-deployer/main/xray_cf_lite.sh
...
```
1. 安装节点        - 部署 xray + 配置 CF（DNS/SSL/Origin Rules）+ 生成订阅
2. 卸载            - 停止 xray + 完整回滚 CF 配置
3. 查看订阅        - 显示上次生成的订阅链接
4. 修改配置        - 修改 UUID / 端口 / WS 路径（可单改或全改）
5. 查看当前配置    - 显示域名、UUID、端口映射、服务状态
6. 更新外部端口    - NAT 换端口专用，只更新 CF Origin Rules，不重启 xray
```

## NAT 端口映射

脚本自动检测 NAT 环境（内网 IP）。安装时输入端口映射，格式为 `内部端口:外部端口`：
...
外部端口变化时使用菜单 6 一键更新，不需要重启 xray。

## 安装时可自定义

- **UUID**：留空自动生成，或输入自定义 UUID
- **端口**：留空随机分配，NAT 模式下输入映射
- **WS 路径前缀**：留空使用 UUID 前 8 位，或自定义
- **协议**：vless / trojan / vmess，可选一个或多个
...
| 状态记录 | `/etc/xray-cf-lite/state.json` |
| CF 凭据 | `/etc/xray-cf-lite/cf_account.json` |
| 订阅快照 | 运行目录下 `cf_lite_last_links.txt` |
| OpenRC 服务 | `/etc/init.d/xray`（非 systemd 环境） |

## 注意事项

- 若 Cloudflare 域名开启了 **Bot Fight Mode**，需添加 WAF 跳过规则或关闭，否则 WebSocket 连接会被拦截（返回 403）
- SSL 模式会被设为 `flexible`，卸载时恢复原值
- 订阅链接使用 `yx-auto.pages.dev` 服务，参数格式与主脚本一致
